...
let nameAliasesDict = Dictionary(uniqueKeysWithValues: nameAliases.map { ($0.oldName, $0.newName) })
...
let legacyAliasesSet = Set(legacyAliases.map { $0.legacyName })
...
let symbolManifestByName = Dictionary(uniqueKeysWithValues: symbolManifest.map { ($0.name, $0) })
...
nameAliases = nameAliases.filter { alias in !legacyAliasesSet.contains(alias.oldName) }
...
func allAliases(for symbolName: String, includeSelf: Bool = true) -> [ScannedSymbol] {
    ...
    if olderAliases.isNotEmpty {
        result += olderAliases
    } else if let newestAlias = nameAliasesDict[symbolName] {
        result += nameAliases.filter { $0.newName == newestAlias && $0.oldName != symbolName }.map(\.oldName) + [newestAlias]
    }
    return result.compactMap { symbolManifestByName[$0] }
...
let availabilityKeys = Array(groupedSymbols.keys)
var availabilityExtensions = Array(repeating: "", count: availabilityKeys.count)
let queue = DispatchQueue(label: "codegen.concurrent", attributes: .concurrent)
let group = DispatchGroup()

for (i, availability) in availabilityKeys.enumerated() {
    group.enter()
    queue.async {
        var outputString = "// Don't touch this manually, this code is generated by the SymbolsGenerator helper tool\n\n"
        ...
        outputString += groupedSymbols[availability]!.map(symbolToCode).joined(separator: "\n\n")
        outputString += "\n}\n"
        availabilityExtensions[i] = outputString
        group.leave()
    }
}
group.wait()

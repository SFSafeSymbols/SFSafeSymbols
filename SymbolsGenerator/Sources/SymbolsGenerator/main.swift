...
var caseIterableExtension: String = {
    ...
    outputString += "var result: [SFSymbol] = []\n\t\t"

    let body: String = groupedSymbols.sorted(by: {$0.key < $1.key}).map { availability, symbols in
        var bodyString = "if #available(iOS \(availability.iOS), macOS \(availability.macOS), tvOS \(availability.tvOS), watchOS \(availability.watchOS), *) "
        bodyString += "{\n"
        bodyString += "\t\t\t// MARK: - \(availability.version) Symbols\n"

...
let groupedAllLatestSymbolsFileContents: Dictionary<Availability, String> = groupedAllLatestSymbols.reduce(into: [:]) {
    ...
    outputString += "\tinternal static var allSymbols\(versionUnderscored): Array<SFSymbol> = [\n"
...
let allSymbolsExtension: String = {
    ...
    outputString += "\tpublic static var allSymbols: SymbolSet = {\n"
    outputString += "\t\t"
    outputString += groupedAllLatestSymbolsFileContents.keys.sorted().map { availability in
        ...
        bodyString += "{\n"
        bodyString += "\t\t\treturn SymbolSet(elements: allSymbols\(versionUnderscored))\n"
        return bodyString
...
let symbolSetExtension: String = {
    let codeFor: (Localization) -> String = { loc -> String in
        ...
        output += "\tvar \(loc.variableName): [SFSymbol] {\n\t\tmap { symbol in\n"
...
SFFileManager.write(symbolSetExtension,
                    to: outputDir.appendingPathComponent("SymbolSet+Localizable.swift"))

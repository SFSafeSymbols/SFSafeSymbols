...
let groupedAllLatestSymbolsFileContents: Dictionary<Availability, String> = groupedSymbols.reduce(into: [:]) {
    ...
    outputString += "\tinternal static var allSymbols\(availability.versionUnderscored): Set<SFSymbol> {\n"
    outputString += "\t\t["
    outputString += symbols.map { "\n\t\t\t" + $0.propertyName + "," }.joined() + "\n"
    outputString += "\t\t]\n"
    outputString += "\t}\n"
...
let allSymbolsExtension: String = {
    ...
    outputString += "\t\tvar result = allSymbols\(availabilities.removeFirst().versionUnderscored)\n"
    ...
    outputString += availabilities.map { availability in
        ...
        bodyString += "{\n"
        bodyString += "\t\t\tresult.formUnion(allSymbols\(availability.versionUnderscored))\n"

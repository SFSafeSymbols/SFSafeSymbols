...
public extension SwiftUI.Image {
    ...
    init(systemSymbol: SFSymbol) {
        if systemSymbol.rawValue.hasPrefix(FALLBACK_PREFIX) {
            self.init(String(systemSymbol.rawValue.dropFirst(FALLBACK_PREFIX.count)), bundle: Bundle.module)
        } else {
...
// AppIntents serves as a placeholder SDK to check if the iOS 16.0, macOS 13.0, ... SDKs are available
    ...
    init(systemSymbol: SFSymbol, variableValue: Double?) {
        if systemSymbol.rawValue.hasPrefix(FALLBACK_PREFIX) {
            self.init(String(systemSymbol.rawValue.dropFirst(FALLBACK_PREFIX.count)), variableValue: variableValue)
        } else {
